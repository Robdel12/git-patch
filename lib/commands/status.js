...
export function run({ json = false } = {}) {
  ...
  console.log(`Staged:     ${stagedHunkCount} hunk(s) across ${stagedFiles.length} file(s)`);
  console.log(`Unstaged:   ${unstagedHunkCount} hunk(s) across ${unstagedFiles.length} file(s)`);
  ...
  let unstagedByFile = new Map(unstagedFiles.map((f) => [f.file, f.hunks.length]));
