 ...
 */
export function buildPatchFromLines(fileDiffs, hunkId, changeLineIndices, mode = "stage") {
...
function buildFileHeader(file) {
  let oldFile = file.oldFile || `a/${file.file}`;
  let newFile = file.newFile || `b/${file.file}`;
  return `diff --git ${oldFile} ${newFile}\n--- ${oldFile}\n+++ ${newFile}`;
}
